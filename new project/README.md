# new project — LSRG-ByteTrack 研究方向工作区
...
| 文档 | 内容 |
|---|---|
| `开发进度.md` | 进度台账：各分析步骤状态、产物位置、复现命令、下一步建议 |
| `COE.md` | 跨对话经验：环境、数据口径、历史教训 —— **新会话先读这个** |
| `day1_report.md` | 研究报告一：失效分类、设计收缩与几何门控基线 |
| `day2_report.md` | 研究报告二：候选机制离线筛选（V8）与 DEN 在线插桩验证 |
| `diag_exp/day3_report.md` | 研究报告三（独立工作区）：可观测性诊断实验与 Go/No-Go 裁决（情况 B） |
| `研究方向.md` | 研究目标与技术路径（当前版本含 Go/No-Go 判据） |
...
└── diag_exp/          Day 3 诊断实验独立工作区（自包含，只读 data/ 与 taxonomy/）：
                       ...
                       day3_report.md                  研究报告三：可观测性诊断与 Go/No-Go 裁决（情况 B）
                       README.md                       工作区索引与复现
```
...
- **所有代码与产物统一在本文件夹**：脚本在 `code/` 原位运行（`_repo_root()` 自动探测仓库根），产物直接写入 `taxonomy/`，无双位置
- 主仓库 `E:\科研\ByteTrack` 侧只作**只读数据源**：`datasets/` GT、`YOLOX_outputs/*_v001_full/track_results/`、`YOLOX_outputs/analysis/taxonomy/`（冻结输入）
- `tools/` 保留跟踪运行入口 `track_v001.py`（在线插桩阶段使用）与数据集转换/可视化/诊断工具；V1/V2 事件分析脚本已删除（产物冻结）
- 注意：主仓库**不是 git 仓库**，无版本回滚能力，文件操作需谨慎
- **报告与代码解耦**：`taxonomy/event_taxonomy_report.md` 与根目录 `day{1,2}_report.md` 是手写静态文档，改报告直接编辑 md，不需要重跑代码
...
- 细节见 `taxonomy/event_taxonomy_report.md`（事件侧）、`day1_report.md`（设计收敛与门控基线）、`day2_report.md`（三机制筛选与在线插桩）、`diag_exp/day3_report.md`（可观测性诊断与路线裁决）
