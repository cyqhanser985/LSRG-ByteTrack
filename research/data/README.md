...
**定位**：本目录是整条分析链的**唯一权威冻结输入**。所有文件为 V1/V2 阶段事件分析脚本（已删除）的冻结产物快照，复制自主仓库 `YOLOX_outputs/analysis/`（见 COE.md §2）。脚本（`code/analysis.py`、`code/den_online_eval.py`、`diag_exp/run_diag.py`）从这里**只读**数据，**不写入**。

> **Canonical 声明**：`research/data/` 是后续所有实验与分析代码的唯一读取来源。`YOLOX_outputs/analysis/` 与 `reports/ByteTrack_ID分析报告/assets/tables/` 下对应的 CSV 仅为历史归档镜像，不再作为分析输入。任何新代码不得绕过 `research/data/` 直接读取旧镜像。
...
- `SHA256SUMS.txt` 记录了本目录全部 6 个 CSV 的 SHA-256 校验值。修改任何文件后必须重新生成并提交该文件；校验命令示例：
...
- 本目录不存放报告（报告在 `reports/day{1,2,3}_report.md` 与 `taxonomy/event_taxonomy_report.md`）。
