...
> 进度看《docs/开发进度.md》，研究结论看《reports/day1_report.md》《reports/day2_report.md》《reports/day3_report.md》，本文件只讲"怎么干、别踩什么坑"。
...
| 版本控制 | ✅ **Git 仓库，`baseline-v001` 为重构前基线** | 文件操作仍应先看内容；删除/覆盖可通过 Git 恢复 |
...
- **新增口径**（2026-08-07/08，详见 `taxonomy/event_taxonomy_report.md` 与 `reports/day1_report.md` / `reports/day2_report.md`）：
  ...
  - **DEN 在线插桩口径**（2026-08-08 起，见 `reports/day2_report.md` §二/§三）：
...
4. 旧脚本的 `ROOT = dirname(dirname(abspath(__file__)))` 硬编码仓库布局（V1/V2 遗留，仍在 `tools/` 运行）；**V3–V6 脚本已改为 `_repo_root()` 自动探测**（从 `research/code/` 或 `tools/` 运行均可）
...
- 研究文档（开发进度 / COE / README / day1-day3 报告）：中文、UTF-8、GitHub Flavored Markdown
- 脚本注释：英文、纯 ASCII（历史约定，避免编码问题）；报告产物用英文
- 术语统一：冷启动接管 S_c / 活跃接管 S_r / 历史重新激活 S_h；LSRG-ByteTrack 已收缩为"分类诊断 → 轻量风险判断 → S_r 局部纠错 / 保持 ByteTrack"最小闭环（2026-08-07，原 Diagnose → Gate → Route → Resolve 四阶段与三头 Gate 因 V3–V6 裁决移除，见《reports/day1_report.md》第二、三节）

## 7. 常用命令速查

```bash
# 唯一的分析入口（research/code/analysis.py 原位运行，产物写入 research/taxonomy/；
# 输入为只读：data/ 冻结事件表 + YOLOX_outputs/*_v001_full/track_results/）
python "research/code/analysis.py"                                  # 全量三数据集（约 3 分钟）
python "research/code/analysis.py" --datasets mot17 --ideas kmc     # 冒烟（~15s）
# 跟踪运行（在线插桩阶段入口，tools/ 保留）
python tools/track_v001.py -f exps/example/mot/yolox_x_mot17_v001.py \
    -c pretrained/bytetrack_x_mot17.pth.tar -expn mot17_v001_full -b 1 -d 1 --fp16 --fuse
# 报告是手写静态文档（reports/day1_report.md / reports/day2_report.md / reports/day3_report.md + taxonomy/event_taxonomy_report.md）：改报告直接编辑 md，不重跑代码
```
